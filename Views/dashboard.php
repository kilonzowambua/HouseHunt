...
<?php if ($user_type == 'Administrator') { 
 $user_id='NULL';
 require_once('../Helpers/analysis.php');
...
<?php } elseif ($user_type == 'House Owner') { 
    $user_id=$userID;
    require_once('../Helpers/analysis.php');
  ?>
  <body x-data class="is-header-blur" x-bind="$store.global.documentBody">
        ...
        <div class="col-span-12 lg:col-span-4 xl:col-span-3">
          <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 sm:gap-5 lg:grid-cols-1 lg:gap-6">
            <div class="sm:col-span-2 lg:col-span-1">
              ...
              <div class="mt-3 grid grid-cols-1 gap-4 sm:grid-cols-2 sm:gap-x-5 lg:grid-cols-1">
              <?php
$query="CALL manageBooking('get_booking_by_ownerid_limited', null,'$userID', null, null, null, null, null);";
$results1=mysqli_query($mysqli2,$query);
while($booking=mysqli_fetch_object($results1)){
?>
<div class="card p-3">
  <div class="flex items-center justify-between space-x-2">
    <div class="flex items-center space-x-3">
      ...
      <div>
        <p class="font-medium text-slate-700 line-clamp-1 dark:text-navy-100">
        <?php echo $booking->Seeker_name;?>
        </p>
        <p class="mt-0.5 text-xs text-slate-400 line-clamp-1 dark:text-navy-300">
          Status :  <?php echo $booking->booking_status;?>
          <br>
          Date : <?php echo $booking->booking_created_on;?> 
        </p>
      </div>
    </div>
  </div>
